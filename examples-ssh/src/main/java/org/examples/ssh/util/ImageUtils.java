...
import org.examples.ssh.exceptions.CommonsAssistantException;
...
public final class ImageUtils {
	 ...
	 *            the logo text 水印文字
	 * @param fontColor
	 *            the font color 水印文字颜色
	 * @param fontName
	 *            the font name 水印字体
	 * @param fontSize
	 *            the font size 水印字体大小
	 * @param alpha
	 *            the alpha 水印字体透明度
	 * @param pointX
	 *            the point x 水印字体X坐标
	 ...
	 *
	 * @param inputStream
	 *            the input stream 图片文件流
	 * @param logoText
	 *            the logo text 水印文字
	 * @param maxImgWidth
	 *            the max img width 图片重设大小最大宽度
	 * @param fontColor
	 *            the font color 水印文件颜色
	 * @param fontName
	 *            the font name 水印文字字体
	 * @param fontSize
	 *            the font size 水印文字大小
	 * @param alpha
	 *            the alpha 水印文字透明度
	 * @param pointX
	 *            the point x 水印文字x坐标
	 ...
	 *            the degree 水印文字旋转角度
	 * @param targetPath
	 *            the target path 加工后图片保存路径
	 */
	public static void resizeAddLogoWrite(InputStream inputStream, String logoText, int maxImgWidth, Color fontColor,
			String fontName, int fontSize, float alpha, int pointX, int pointY, Integer degree, String targetPath) {
		// try {
		// BufferedImage bufferedImage = ImageIO.read(inputStream);
		// //重设大小
		// BufferedImage resize = resize(bufferedImage, maxImgWidth);
		// //添加水印
		// resize = watermarkText(logoText, fontColor, fontName, fontSize,
		// alpha, pointX, pointY,
		// degree, resize);
		// //转换出的文件
		// File fo = new File(targetPath);
		...
		// fo.getParentFile().mkdirs();
		// }
		// String suffix = fo.getName().substring(fo.getName().lastIndexOf(".")
		// + 1);
	 ...
	 *            指定的缩放宽度
	 * @param newHeight
	 *            指定的缩放高度，0或小于0时将按比例缩放
	 * @param isDoHeight
	 *            最大宽度是否作用于高度(true时即高度不得超过指定的宽度，竖形长图片高度超过宽度时以高度为准)
